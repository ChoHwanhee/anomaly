...
from model.transunet import TransAnomaly 
...
def val(cfg, train_scores=None, models=None, iter=None):
    ...
    '''
    if models:  
        generator = TransAnomaly(1, 4).cuda().eval()
        
        generator.load_state_dict(torch.load(f'weights/latest_{cfg.dataset}.pth')['net_g'])
